...
def write_hostapd_conf(ssid=None, passphrase=None, interface=None, channel=None, hw_mode=None, ieee80211n=None, ieee80211ac=None):
    ...
    ssid = ssid or yaml.config.get('default_ssid', "RaspberryPi4B")
...
def main():
    ...
    if args.setup:
        ...
        interface = args.interface or get_valid_input(f"Enter the network interface for your hotspot (default is {config.get('default_interface', 'wlan1')}): ",
                                lambda x: x or config.get('default_interface', 'wlan1'))
        ip_address = args.ip or get_valid_input(f"Enter the IP address for your hotspot (default is {config.get('default_ip_address', '192.168.2.1/24')}): ",
